...
// index.php?page=audiobook&id=1
$current_page = isset($_GET['p']) && is_numeric($_GET['p']) ? (int)$_GET['p'] : 1;
$stmt = $pdo->prepare('SELECT * FROM audiobooks ORDER BY titlu ASC LIMIT ?,?');
...
<div class="products content-wrapper">
    ...
    <div class="buttons">
        ...
        <a href="index.php?page=audiobooks&p=<?=$current_page-1?>">Prev</a>
        <?php endif; ?>
        <?php if ($total_products > ($current_page * $num_products_on_each_page) - $num_products_on_each_page + count($products)): ?>
        <a href="index.php?page=audiobooks&p=<?=$current_page+1?>">Next</a>
        <?php endif; ?>
    </div>
</div>
