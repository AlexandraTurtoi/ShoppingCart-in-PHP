...
if (isset($_GET['id'])) {
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            $stmt = $pdo->prepare('DELETE FROM audiobooks WHERE id = ?');
            $stmt->execute([$_GET['id']]);
            $msg = 'You have deleted the contact!';
            
        } else {
            // User clicked the "No" button, redirect them back to the read page
            header('Location: listaAudio.php');
            exit;
        }
    }
} else {
...
<div class="content delete" >
    ...
    <div class="yesno" >
        <a href="deleteAudio.php?id=<?=$product['id']?>&confirm=yes" align="center">Yes</a>
        <a href="deleteAudio.php?id=<?=$product['id']?>&confirm=no">No</a> 
    </div>
